...
int newBuffer(circularBuffer *buf, int suggestedSize){
	...
	buf->bufferContent = (char*) malloc ((suggestedSize+1) * sizeof(char)); 
	//reserved one more for null pointer.
	...
	buf->mtx = st_mutex_new(); //creat a mutex, which is 1 at the beginning. 
	return 0;
}
...
bool isConsistent(circularBuffer *buf){ 
	//for debugging only, check if the circulan array is consistent
	if (buf->status==-1) {
