		...
		if (cpid == 0) { //child process
			...
			if (isPath(formattedArgv[0])){ 
				if(stat(formattedArgv[0],&buf)<0){
					printf("The file indicated by the path does not not exist\n Found failuare for path: %s\n", formattedArgv[0]);
					_exit(0);
				}
				else{
					printf("Found via given path name\n");
				...
				if (isFileExistInPaths(formattedArgv[0],searchPool,fullFath)) {
					printf("Found via given search in PATH environment name\n");
					formattedArgv[0] = fullFath; //pointer to 
					execv(formattedArgv[0],formattedArgv);
				}
				else {
					printf("File \"%s\"is not found in the enviroment paths\n",formattedArgv[0]);
					_exit(0);
				}							
			}
			//=========execv() function End =======
			...
			if (signal(SIGINT, SIG_IGN)==SIG_ERR) {
				perror("Failed to install SIGINT ignore handler");
				exit(3);
			}
			if (signal(SIGTSTP, SIG_IGN)==SIG_ERR) {
				perror("Failed to install SIGTSTP ignore handler");
				exit(3);
			...
			if (term_pid == -1) {
				...
				printf("--P6-- Child Complete, Back to parent process: %d \n", getpid());
			}
		}
	}
    return 0;
