...
class User
{
  ...
  public static function findByUID($uid) {
    ...
    if ($result) {
      ...
      return new User($user_info['uid'],$user_info['firstName'],$user_info['lastName'],
		       $user_info['gender'],$user_info['DOB'],$user_info['year'],
  ...
  private function __construct($uid,$first,$last,$gender,$dob,$year,$major,$musicListening,
  ...
  public static function create($uid,$first,$last,$gender,$dob,$year,$major,$musicListening,
  							   $visitors,$cleanness,$sleepTime,$wakeTime,$about,$photo) {
    $mysqli = new mysqli("classroom.cs.unc.edu", "haoyangh", "comp426", "comp42617db");
  	$result = $mysqli->query("INSERT INTO User (`uid`, `firstName`, `lastName`, `gender`, `DOB`, `year`, `musicListening`, `visitors`, `cleanness`, `sleepTime`, `wakeTime`, `aboutMe`, `profilePhotoURL`) VALUES('" .
  			$uid . "', '" . $first . "', '" . $last . "', '" . $gender .  "', '" . $dob .  "', '" . $year .
  	  	  "', '" . $musicListening . "', '" . $visitors . "', '" . $cleanness . "', '" . $sleepTime . 
  	  	 "', '".$wakeTime."', '".about. "', '". $photo ."')");

  	if ($result) {
  		return new User($uid,$first,$last,$gender,$dob,$year,$major,$musicListening,
  							   $visitors,$cleanness,$sleepTime,$wakeTime,$about,$photo);
  	}
		return null;
    }
  ...
  //POST user.php/<id>
  public function updateUserInfo($new_first,$new_last,$new_gender,$new_dob,$new_year,$new_major,$new_music,
  ...
  private function update() {
    $mysqli = new mysqli("classroom.cs.unc.edu", "haoyangh", "comp426", "comp42617db");
  	$result = $mysqli->query("Update User set firstName ='" . $this->first . "', lastName = '" . $this->last . "', gender = '" . $this->gender .
  			"', DOB ='" . $this->dob . "', year='" . $this->year . "', musicListening='" . $this->musicListenging . 
  			"', visitors='" . $this->visitors . "', cleanness='"  . $this->cleanness . "', sleepTime='" . $this->sleepTime .
  			"', wakeTime='" . $this->wakeTime . "', aboutMe='" . $this->about . "', profilePhotoURL='" . $this->photo . "' where uid = '" . $this->uid."'");
  	return $result;
  }
}
?>